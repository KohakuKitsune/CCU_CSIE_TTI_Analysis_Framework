...
save_path = 'C:\\CSIE_Project\\Program\\GRAPH\\Type256'
...
for filename in os.listdir(dir_path):
    if filename.endswith(".csv"):
        ...
        for name, group in df.groupby(df['Date'].dt.to_period('M')):
            ...
            plt.plot(group['Date'], group['TTI Value Type 2'], 'g', label="TTI Value Type 2\nPeak :當日最高車流量的時段所對應平均速度\nFreeflow :交通部規定之路段的速限(50km/h)",)
            plt.plot(group['Date'], group['TTI Value Type 4(AM)'], 'm', label="TTI Value Type 4(AM)\nPeak :sharpness\nFreeflow :交通部規定之路段的速限(50km/h)")
            plt.plot(group['Date'], group['TTI Value Type 4(PM)'], 'c', label="TTI Value Type 4(PM)\nPeak :sharpness\nFreeflow :交通部規定之路段的速限(50km/h)")
            # set x axis scale
