...
save_path = 'C:\\CSIE_Project\\Program\\GRAPH\\Type134'
...
for filename in os.listdir(dir_path):
    if filename.endswith(".csv"):
        ...
        for name, group in df.groupby(df['Date'].dt.to_period('M')):
            ...
            plt.plot(group['Date'], group['TTI Value Type 1'], 'k', label="TTI Value Type 1\nPeak :當日最高車流量的時段所對應平均速度\nFreeflow :當日最高平均時速") 
            plt.plot(group['Date'], group['TTI Value Type 3(AM)'], 'r', label="TTI Value Type 3(AM)\nPeak :sharpness\nFreeflow :當日最高平均時速")
            plt.plot(group['Date'], group['TTI Value Type 3(PM)'], 'b', label="TTI Value Type 3(PM)\nPeak :sharpness\nFreeflow :當日最高平均時速")
            # set x axis scale
